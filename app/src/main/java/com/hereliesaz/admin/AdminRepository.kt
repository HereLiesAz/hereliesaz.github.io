...
class AdminRepository(private val api: GitHubApi) {
    ...
    suspend fun saveMetaEntry(id: String, entry: PaintingMeta?) {
        ...
        if (entry == null) root.remove(id) else root.put(id, JSONObject().apply {
            put("title", entry.title); put("description", entry.description); put("tags", JSONArray(entry.tags)); put("forSale", entry.forSale)
            if (entry.forSale && entry.price != null) { put("price", entry.price); put("currency", entry.currency) }
        })
        api.putFile(META_PATH, (root.toString(2) + "\n").toByteArray(), "admin android: update metadata for $id", file?.sha)
    }
    suspend fun loadBandOverrides(): Map<String, Set<Int>> {
        ...
        return root.keys().asSequence().associateWith { id ->
            ...
            buildSet { for (i in 0 until hidden.length()) add(hidden.getInt(i)) }
        }
    }
    suspend fun saveBandOverrideEntry(id: String, hidden: Set<Int>?) {
        val file = api.getFile(BAND_OVERRIDES_PATH)
        val root = parseObject(file?.content)
        if (hidden.isNullOrEmpty()) root.remove(id) else root.put(id, JSONObject().put("hidden", JSONArray(hidden.sorted())))
        api.putFile(BAND_OVERRIDES_PATH, (root.toString(2) + "\n").toByteArray(), "admin android: update band overrides for $id", file?.sha)
    }
    suspend fun loadSiteContent(): SiteContent {
        ...
        return runCatching {
            ...
            SiteContent(root.optString("about", SiteContent.DEFAULT_ABOUT), if (links.isEmpty()) SiteContent.DEFAULT_LINKS else links)
        }.getOrDefault(SiteContent())
    }
    suspend fun saveSiteContent(content: SiteContent) {
        val file = api.getFile(SITE_PATH)
        val links = JSONArray()
        content.menuLinks.forEach { links.put(JSONObject().put("label", it.label).put("href", it.href).put("external", it.external)) }
        val root = JSONObject().put("about", content.about).put("menuLinks", links)
        api.putFile(SITE_PATH, (root.toString(2) + "\n").toByteArray(), "admin android: update site content", file?.sha)
    }
    suspend fun fetchTheaterMeta(id: String): TheaterMeta? {
        val encoded = GitHubApi.encodeSegment(id)
        val text = runCatching { api.publicText("$PUBLIC_BASE/data/theater/$encoded.theater.json") }.getOrNull() ?: return null
    ...
    suspend fun loadBitmap(url: String, maxDimension: Int = 1024): Bitmap? = withContext(Dispatchers.Default) {
        ...
        if (largest <= maxDimension) decoded else {
            ...
            Bitmap.createScaledBitmap(decoded, max(1, (decoded.width * scale).toInt()), max(1, (decoded.height * scale).toInt()), true)
                .also { if (it !== decoded) decoded.recycle() }
        }
    }

    suspend fun buildBandPreviews(id: String, meta: TheaterMeta, hidden: Set<Int>): List<BandPreview> = withContext(Dispatchers.Default) {
        ...
        val depth = if (rawDepth.width == painting.width && rawDepth.height == painting.height) rawDepth
        else Bitmap.createScaledBitmap(rawDepth, painting.width, painting.height, true).also { if (it !== rawDepth) rawDepth.recycle() }
        val count = painting.width * painting.height
        val p = IntArray(count); val d = IntArray(count)
        ...
        val result = meta.bandCenters.indices.map { band ->
            val lo = meta.bandEdges[band]; val hi = meta.bandEdges[band + 1]; val out = p.copyOf()
            for (i in 0 until count) {
                val z = Color.red(d[i]) / 255.0
                if (z < lo || z >= hi) out[i] = out[i] and 0x00FFFFFF
            }
            BandPreview(band, lo, hi, Bitmap.createBitmap(out, painting.width, painting.height, Bitmap.Config.ARGB_8888), band in hidden)
        }
        painting.recycle(); depth.recycle(); result
    }

    suspend fun uploadPainting(item: PickedFile): String {
        val safe = sanitizeIdAndFilename(item.displayName)
        api.putFile("public/assets/" + safe.filename, item.bytes, "admin android: add painting " + safe.id)
        return safe.id
    }
    ...
    private suspend fun waitForNoActiveRemovalRun() {
        ...
        while (true) {
            ...
            if (System.currentTimeMillis() > deadline) error("A previous removal run has been active for over 20 minutes. Check GitHub Actions.")
            delay(REMOVAL_POLL_MS)
        }
    }
    private fun parseMeta(text: String): Map<String, PaintingMeta> {
        ...
        return root.keys().asSequence().associateWith { id ->
            val x = root.optJSONObject(id) ?: JSONObject(); val a = x.optJSONArray("tags") ?: JSONArray()
            PaintingMeta(x.optString("title"), x.optString("description"), List(a.length()) { a.optString(it) }, x.optBoolean("forSale", false), if (x.has("price")) x.optDouble("price") else null, x.optString("currency", "USD"))
        }
    }
